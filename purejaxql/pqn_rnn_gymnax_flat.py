...
class Config(purejaxql.pqn_gymnax_flat.Config, TypedDict):
...
def _update_step(
    runner_state: tuple[
        CustomTrainState, Transition, ExplorationState[TEnvState], dict, chex.PRNGKey
    ],
    unused,
    network: Static[RNNQNetwork],
    config: Static[dict],
...
def _learn_epoch(
    ...
    rng, _rng = jax.random.split(rng)
    (train_state, rng), (loss, qvals) = jax.lax.scan(
        partial(_learn_phase, config=config, network=network),
        (train_state, rng),
        minibatches,
    )
...
def _compute_targets(
    ...
    _, targets = jax.lax.scan(
        partial(_get_target, gamma=gamma, lambda_=lambda_),
